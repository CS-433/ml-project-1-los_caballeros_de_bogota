...
def filter_data(y, x, ids):
    """Filter the data by DER_mass_MMC and PRI_jet_num"""
    ...
    # Filter by DER_mass_MMC (defined or undefined):
    mask = x[:, 0] == -999.0
    x_no_mass = x[mask, :]
    y_no_mass = y[mask]
    ...
    data["mass"]["jet0"]["x"] = np.delete(
        data["mass"]["jet0"]["x"], [4, 5, 6, 12, 22, 23, 24, 25, 26, 27, 28, 29], axis=1
    )
    data["no_mass"]["jet0"]["x"] = np.delete(
        data["no_mass"]["jet0"]["x"],
    ...
    # Delete useless features:
    data["mass"]["jet1"]["x"] = np.delete(
        data["mass"]["jet1"]["x"], [4, 5, 6, 12, 22, 26, 27, 28], axis=1
    )
    data["no_mass"]["jet1"]["x"] = np.delete(
        data["no_mass"]["jet1"]["x"], [0, 4, 5, 6, 12, 22, 26, 27, 28], axis=1
    )
    ...
    data["no_mass"]["jet2"]["x"] = np.delete(
        data["no_mass"]["jet2"]["x"], [0, 22], axis=1
    )
    ...
    data["no_mass"]["jet3"]["x"] = np.delete(
        data["no_mass"]["jet3"]["x"], [0, 22], axis=1
    )
...
def clean_data(x, mod):
    # Replace undefined values by mean of the features (excluding -999.0):
    if mod == "mean":
        mask = x == -999.0
        x[mask] = 0
    ...
    # Replace undefined values by median of the features (excluding -999.0):
    if mod == "med":
        mask = x == -999.0
        x[mask] = 0
